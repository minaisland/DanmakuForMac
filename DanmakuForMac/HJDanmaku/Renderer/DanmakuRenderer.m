...
@property (nonatomic, weak) NSView *canvas;
...
{
    NSTextField *cacheLabel = danmaku.label;
    if (cacheLabel) {
...
- (void)createLabelForDanmaku:(DanmakuBaseModel *)danmaku
{
    ...
    if (_cacheLabels.count<1) {
        ...
        danmaku.label.backgroundColor = [NSColor clearColor];
    } else {
...
- (void)rendererDanmakuLabel:(DanmakuBaseModel *)danmaku
{
    ...
    danmaku.label.alphaValue = 1;
    danmaku.label.font = [NSFont systemFontOfSize:danmaku.textSize];
    danmaku.label.stringValue = danmaku.text;
    danmaku.label.textColor = danmaku.textColor;
    danmaku.label.underLineEnable = danmaku.isSelfID;
    [danmaku.label sizeToFit];
}
...
- (void)rendererDanmaku:(DanmakuBaseModel *)danmaku
{
    ...
    if (danmaku.isShowing) {
        [NSAnimationContext runAnimationGroup:^(NSAnimationContext * _Nonnull context) {
            context.duration = danmaku.remainTime;
            context.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear];
            danmaku.label.animator.frame = CGRectMake(-danmaku.size.width, danmaku.py, danmaku.size.width, danmaku.size.height);
        } completionHandler:^{
